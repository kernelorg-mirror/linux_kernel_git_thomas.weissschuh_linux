...
non_llvm_architectures="alpha m68k powerpc powerpc32 powerpcle sh sparc64 riscv32 arm"
architectures="$*"
...
options="--kunitconfig kernel/time --kconfig_add CONFIG_PRINTK_TIME=y"
...
for arch in $architectures; do
	...
	echo ./tools/testing/kunit/kunit.py run --build_dir $build_dir --arch $arch $compile_opts $options $filter
